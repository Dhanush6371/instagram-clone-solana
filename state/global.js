...
import { getLikeAccountPk, getPostAccountPk, getProgram, getUserAccountPk } from "../utils";
...
export const GlobalState = ({ children }) => {
  ...
  useEffect(() => {
    if (!posts) {
      // Fetch posts if posts don't exist
      fetchPosts();
    }
  }, [posts, fetchPosts]);
  ...
  const createUser = useCallback(async () => {
    ...
    try {
      ...
      toast.error("You don't have enough SOL and you need more than 1 SOL", e.message)
    }
  }, [program]);
